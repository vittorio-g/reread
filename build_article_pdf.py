"""
build_article_pdf.py
Reads article_assets/article_data.pkl + figures and produces:
  C:\\Users\\vitto\\Downloads\\ReReReRe_Article_2026-04-26.pdf
...
ASSETS    = os.path.join(ROOT, "article_assets")
OUT_PDF   = r"C:\Users\vitto\Downloads\ReReReRe_Article_2026-04-26.pdf"
...
mcc_30  = float(mA_by_size.loc[min(D["sizes"]), "mcc"])
...
S += [
    ...
    Paragraph(("A permutation-based individual-correlation method, "
              ...
              styles["Subtitle"]),
    Spacer(1, 0.4*cm),
    B(f"Working draft, {datetime.date.today().isoformat()}"),
    B(f"Simulation: {D['n_runs']} independent runs across "
      f"{len(D['sizes'])} questionnaire sizes "
      f"({min(D['sizes'])}–{max(D['sizes'])} items: {sizes_str}), "
      f"{len(D['rates'])} careless rates "
      f"({rates_str}), "
      f"5 replications each, n = 500 respondents per dataset."),
    Spacer(1, 0.4*cm),
]
...
S += [
    ...
    BOX(
        ...
        f"length: from MCC ≈ {mcc_30:.2f} on the shortest ({min(D['sizes'])}-item) "
        f"questionnaire to MCC ≈ {mcc_max:.2f} on the largest "
        f"({max(D['sizes'])}-item) battery, with no parameter tuning required."
    ),
    Spacer(1, 0.2*cm),
    PageBreak(),
]
...
S += [
    ...
    CAP("Figure 1. MCC at FPR = 5% as a function of total questionnaire items, "
        "averaged over careless rates and replications. Error bars: ±1 SD across "
        "replications. Both scenarios benefit roughly equally from longer "
        "questionnaires."),
]
...
S += [
    ...
    CAP("Figure 2. All twelve metrics (sensitivity, specificity, PPV, NPV, F1, F2, "
        ...
        "Lines: mean across replications; error bars: ±1 SD."),
    PageBreak(),
]
...
S += [df_to_table(dfA.round(3), col_widths=[1.6*cm]*8),
      CAP("Table 2. Scenario A (τ=0.60) performance by total items. "
          "Means across 2 careless rates × 5 replications = 10 runs each."),
      Spacer(1, 0.2*cm),
]
...
S += [
    ...
    BOX(f"<b>Effect size of ReReReRe.</b> Pooled across all sizes and rates: "
        ...
        f"is the minimal defensible ReReReRe-aware ensemble.</i>"),
    PageBreak(),
]
...
S += [
    ...
    B("Why does ReReReRe help? Because the auxiliary detectors fail in the very "
      "regimes where it succeeds. Figure 4 shows detection rate as a function of "
    ...
    CAP("Figure 4. Per-pattern detection in Scenario A. Auxiliary detectors "
...
S += [
    ...
    img("fig06_heatmap.png", width_cm=12.5),
    CAP("Figure 5. MCC heatmap (Scenario A). Performance increases monotonically "
        "with both items and sample careless rate. Higher rates are easier "
        "because the positive class is better represented for the classifier."),
    Spacer(1, 0.2*cm),
    img("fig05_score_distribution.png", width_cm=15),
    CAP("Figure 6. Distribution of the iterative ReReReRe z-score (sign-flipped "
...
S += [
    ...
    B("ReReReRe is designed for <b>multi-construct questionnaires</b> where the "
      ...
      "responding is not."),
    B("Use it when:"),
    B("• You have at least <b>~50 items spanning ≥6 latent factors</b>. "
      "MCC reaches usable values at 30 items (~0.5–0.6) but is at its best "
      "from 100 items upwards."),
