# Sim_Article_Big.R
# Big simulation collecting data for the ReReReRe article.
#
# DESIGN
#   - 5 questionnaire sizes (30, 64, 100, 150, 200 items)
#   - 2 careless rates in the sample (0.20, 0.40)
...
TAU_A <- 0.60   # Scenario A operational definition
TAU_B <- 0.40   # Scenario B operational definition
...
for (sz in SIZES) {
  for (cr in CARELESS_RATES) {
    for (rep_id in 1:REPS) {
      idx <- idx + 1
      seed <- SEED_BASE + (rep_id - 1) * 1000 + sz$total + as.integer(cr * 100)
      set.seed(seed)
      ...
      # 3. Standard RR (no variance penalty -> raw signal for the article)
      step_t <- Sys.time()
      rr <- tryCatch(
      ...
      # Assemble per-respondent feature matrix
      # NOTE: signs flipped so that HIGHER value = MORE careless for ALL features
      rec <- data.frame(
        size       = sz$total,
        nF         = sz$nF,
        ipf        = sz$ipf,
      ...
      # 6. Train 5-fold RF and evaluate at both scenarios A and B
      ...
      for (sc in c("A", "B")) {
        ...
        # Per-pattern detection at tau_op
      ...
      # Save incrementally so we don't lose work
      if (idx %% 5 == 0) {
        write.csv(do.call(rbind, scores_all),  "sim_article_scores.csv",  row.names = FALSE)
        write.csv(do.call(rbind, metrics_all), "sim_article_metrics.csv", row.names = FALSE)
        write.csv(do.call(rbind, ablation_all),"sim_article_ablation.csv",row.names = FALSE)
        write.csv(do.call(rbind, perpat_all),  "sim_article_perpat.csv",  row.names = FALSE)
