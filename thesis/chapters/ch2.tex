\chapter{Statistical Preliminaries}
\label{ch:stats}
...
The chapter falls into two halves. The first lays the \emph{psychometric}
foundations on which any discussion of questionnaire data rests---measurement
scales, classical test theory, the latent-variable models of factor analysis
and item response theory, and the twin criteria of reliability and
validity---and shows how careless responding threatens each. The second
introduces the \emph{statistical and machine-learning} tools the detector
itself is built from: permutation tests, the bootstrap, the Random Forest,
cross-validation, and the metrics used to judge a classifier.
...
The oldest and still most widely used framework for thinking about test scores
is \emph{classical test theory} (CTT), axiomatised by
\textcite{novick1966axioms} and given its definitive treatment by
\textcite{lordnovick1968statistical}. CTT decomposes an observed score into two
additive parts,
\begin{equation}
X \;=\; T \;+\; E,
\label{eq:ctt}
\end{equation}
where $X$ is the score a respondent actually obtains, $T$ is their \emph{true
score}---the score they would obtain on average over infinitely many
independent administrations---and $E$ is \emph{random measurement error}. The
theory's defining assumptions are that error has expected value zero, is
uncorrelated with the true score, and is uncorrelated across independent
measurements; the observed-score variance then partitions cleanly into true and
error variance, $\mathrm{Var}(X) = \mathrm{Var}(T) + \mathrm{Var}(E)$.

\emph{Reliability} is defined as the proportion of observed variance that is
true variance,
\[
\rho_{XX'} \;=\; \frac{\mathrm{Var}(T)}{\mathrm{Var}(X)}
\;=\; 1 - \frac{\mathrm{Var}(E)}{\mathrm{Var}(X)},
\]
a number between $0$ (all noise) and $1$ (no error); the related
\emph{standard error of measurement}, $\mathrm{SEM} = s_X\sqrt{1-\rho_{XX'}}$,
expresses the same idea on the score scale. CTT operates at the level of the
\emph{total score}; the latent-variable models of the next two sections instead
model responses item by item.

Classical test theory gives the central problem of this thesis its sharpest
formulation. A careless response carries little or no true-score information:
in the limit of purely random responding, $X$ is essentially all $E$ and no
$T$. Careless respondents therefore inflate $\mathrm{Var}(E)$, which by
definition lowers reliability, attenuates the correlations a scale was built to
measure (Section~\ref{sec:reliability}), and biases any statistic computed on
the contaminated data. Detecting careless responding is, in this language, the
task of identifying the respondents whose observed scores are dominated by
error rather than by true score---before they degrade the measurement for
everyone else.

\section{Item Response Theory}
\label{sec:irt}

Classical test theory treats the total score as the unit of analysis and is
silent about individual items. \emph{Item response theory} (IRT) takes the
complementary view, modelling the probability of each item response as a
function of a respondent's latent trait \parencite{embretson2000item,
hambleton1991fundamentals}. In the simplest model for a dichotomous item, the
probability that a respondent with latent trait $\theta$ endorses item $j$ is a
logistic function of the gap between the trait and the item's \emph{difficulty}
$b_j$, scaled by the item's \emph{discrimination} $a_j$,
\begin{equation}
P(X_j = 1 \mid \theta) \;=\;
\frac{1}{1 + e^{-a_j(\theta - b_j)}} .
\label{eq:irt}
\end{equation}
The resulting S-shaped \emph{item characteristic curve} rises from near zero for
low-trait respondents to near one for high-trait respondents. IRT originates
with the \textcite{rasch1980probabilistic} model (a one-parameter special case)
and was developed for practical testing by \textcite{lord1980applications};
\textcite{edwards2009introduction} gives an accessible introduction for
psychologists and \textcite{reise2009item} reviews its use in clinical
measurement.

IRT matters here for one reason in particular: it furnishes a principled notion
of an \emph{aberrant} response pattern. Because the model specifies the
probability of every response given the trait, it can quantify how
\emph{surprising} a respondent's particular pattern is, and large surprise is a
signature of misfit. This is the basis of the \emph{person-fit} statistics
\parencite{meijer2001methodology} introduced in Chapter~1 as one family of
careless-detection methods: a careless respondent answers in ways the
measurement model deems improbable, and person-fit indices flag exactly that.
ReReReRe pursues the same goal---finding respondents who violate the structure
the rest of the sample obeys---but does so through cross-item correlation rather
than a fitted IRT model, trading the model's precision for robustness when the
model is unknown or hard to estimate.
...
Classical test theory defines reliability but does not say how to estimate it
from a single administration. The most widely reported estimate is
\emph{coefficient alpha} \parencite{cronbach1951coefficient}, which uses the
average inter-item correlation, adjusted for the number of items, to approximate
the proportion of true variance in a multi-item scale; it ranges between $0$ and
$1$, and high values indicate that the items ``hang together'' as they should if
they measure a common construct.

Alpha is convenient but rests on a strong assumption---that all items are
\emph{essentially tau-equivalent}, contributing equal true-score
information---that real scales rarely meet, in which case alpha can under- or
over-estimate reliability. A substantial methodological literature now
recommends \emph{coefficient omega} \parencite{mcdonald1999test}, which derives
reliability from a fitted factor model and so does not require tau-equivalence
\parencite{sijtsma2009use, revellezinbarg2009coefficients}. Recent tutorials
advise reporting omega rather than alpha by default
\parencite{hayescoutts2020use, mcneish2018thanks, flora2020your}; this thesis
mentions alpha because it remains ubiquitous, but the argument it makes about
careless responding is indifferent to which reliability coefficient is used.

A second classical result ties reliability directly to the correlations this
thesis cares about. \textcite{spearman1904proof} showed that the correlation
between two measured variables is \emph{attenuated}---pulled toward zero---in
proportion to their unreliability, and derived the \emph{correction for
attenuation} that recovers the disattenuated value. The consequence for careless
responding is immediate: because careless answers add error variance
(Section~\ref{sec:ctt}) and so lower reliability, they attenuate every
correlation computed on the contaminated data. A study that fails to screen
careless respondents does not merely add noise; it systematically understates
the very relationships it set out to find.

Reliability thus connects directly to the detection problem. Because careless
answers behave like added measurement error---raising the $\varepsilon_j$ term
of the factor model in Equation~\ref{eq:cfa}, lowering communalities, and
weakening inter-item correlations---a drop in reliability when a scale is
administered to a contaminated sample is one of the symptoms that motivate
detection. Conversely, identifying and removing careless respondents typically
\emph{raises} reliability and sharpens the recovered factor structure, the
practical payoff that Chapter~5 evaluates.

\section{Validity}
\label{sec:validity}

Reliability concerns whether a measurement is reproducible; \emph{validity}
concerns whether it measures what it claims to. The two are distinct---a scale
can be highly reliable yet measure the wrong thing---and validity is the more
fundamental requirement. The modern concept descends from
\textcite{cronbachmeehl1955construct}, who introduced \emph{construct
validity}: the degree to which a score can be interpreted as reflecting the
intended theoretical construct, established by accumulating evidence that the
score behaves as the construct should. \textcite{messick1995validity} recast
validity as a single, unified, evidence-based judgement about the
\emph{interpretation and use} of scores rather than a property of the test in
isolation, and \textcite{borsboom2004concept} pressed the stronger view that a
test is valid only if variation in the attribute actually \emph{causes}
variation in the scores.

Careless responding is, on each of these framings, a direct threat to validity.
When part of a sample answers without engaging item content, the scores no
longer reflect the construct for those respondents, the evidential basis for any
construct interpretation is undermined, and the causal link from attribute to
response is broken. This is why data screening is not mere housekeeping but a
precondition for valid inference \parencite{desimone2015best}: detecting and
removing careless respondents protects the validity of every conclusion drawn
from the remaining data. The detector developed in this thesis is, in this
sense, an instrument for safeguarding construct validity.
