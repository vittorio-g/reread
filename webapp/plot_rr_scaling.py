...
ax.plot(x,d.auc_ens,marker="s",lw=1.8,color=GREY,zorder=2,label="ensemble (rr + partners), for reference")
ax.fill_between(x,d.auc_rr-d.auc_rr_se,d.auc_rr+d.auc_rr_se,color=NAVY,alpha=.16,zorder=2)
ax.plot(x,d.auc_rr,marker="o",lw=2.2,color=NAVY,zorder=3,label="$rr$ index alone")
...
ax.set_title("The $rr$ index scales with questionnaire length",fontsize=11.5)
