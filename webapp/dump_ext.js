...
for(const [name,mf,lf] of SETS){
  ...
  const Mo=[P.header.join(",")]; for(let i=0;i<n;i++)Mo.push(rows[i].map(v=>Number.isFinite(v)?v:"").join(","));
  const Yo=["y,rr,eta"]; for(let i=0;i<n;i++){const eta=W.b0+W.rr*O.rr[i]+g*(W.longstring*O.longstring[i]+W.person_total*O.person_total[i]);
    Yo.push(lab[i]+","+O.rr[i].toFixed(6)+","+eta.toFixed(6));}
  fs.writeFileSync(OUT+"/"+name+"_M.csv",Mo.join("\n"));
  fs.writeFileSync(OUT+"/"+name+"_y.csv",Yo.join("\n"));
  idx.push([name,n,J,(lab.reduce((s,v)=>s+v,0)/n).toFixed(3)]);
