...
for(k in seq_len(nrow(idx))){
  ...
  Y <- read.csv(f)
  lab <- Y$y
  S <- list(ens=Y$eta, rc=Y$rr, PT=Y$pt)
  au <- sapply(S, function(s) aucv(lab,s))
  mc <- sapply(S, function(s) oracle_mcc(lab,s))
  rows[[length(rows)+1]] <- data.frame(dataset=nm, kind=idx$kind[k], n=nrow(Y), J=idx$J[k], rate=mean(lab),
...
write.csv(D, "ext_mcc2.csv", row.names=FALSE)
