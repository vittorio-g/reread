...
/* D only: pi-hat vs true prevalence under the strong-overlap simulation, with the
 * NEW deployed estimator (BIC gate + one-sided attentive-mode fit via R.autoFlag). */
...
for(const pct of [0.05,0.1,0.15,0.2,0.25,0.3]){for(let rep=0;rep<20;rep++){
  ...
  const af=R.autoFlag(e,"standard");outD.push([pct,rep,af.pi.toFixed(4),af.twoComp?1:0]);}}
fs.writeFileSync("sens_pi.csv",outD.map(r=>r.join(",")).join("\n"));console.log("D done (one-sided)");
